...
public class ColorTetrisDriver extends Applet {
   ...
   private class ButtonActionListener implements ActionListener {
      public void actionPerformed(ActionEvent event) {
         ...
         // left swap button was clicked
            ...
            {
               Object right = stacks[colMarker].pop();
               Object left = stacks[colMarker-1].pop();
               stacks[colMarker].push(left);
               stacks[colMarker-1].push(right);
            }
         }
   ...
   public void removeCombos() {
      ...
      for (int i = 0; i < numCols; i++)
      {
         if(i == 0 && stacks[i].height() != 0 && stacks[i+1].height() != 0 && stacks[i+2].height() != 0)
         {
            if(stacks[i].peek().equals(stacks[i+1].peek()) && stacks[i].peek().equals(stacks[i+2].peek()))
            {
               if(stacks[i+3].height() != 0)
               {
                  if(stacks[i].peek().equals(stacks[i+3].peek()))
                  {
                     if (stacks[i+4].height() != 0)
                     {
                        if(stacks[i].peek().equals(stacks[i+4].peek()))
                        {
                           stacks[i+4].pop();
                        }
                     }
         ...
         else if (i == numCols-1 && stacks[i].height() != 0 && stacks[i-1].height() != 0 && stacks[i-2].height() != 0)
         {
            if(stacks[i].peek().equals(stacks[i-1].peek()) && stacks[i].peek().equals(stacks[i-2].peek()))
            {
               if(stacks[i-3].height() != 0)
         ...
         else if (i > 0 && i < numCols-1 && stacks[i].height() != 0 && stacks[i+1].height() != 0 && stacks[i-1].height() != 0)
         {
            if(stacks[i].peek().equals(stacks[i+1].peek()) && stacks[i].peek().equals(stacks[i-1].peek()))
            {
               if(i<numCols-2)
               {
                  if(stacks[i].peek().equals(stacks[i+2].peek()) && stacks[i+2].height() != 0)
                  {
                     stacks[i+2].pop();
                  }
               }
         ...
         if (done == false) {
            ...
            for (int j=0; j<cols; j++) {
               if (stacks[j].height()==0) {}
                  ...
                  Iterator stackRow = stacks[j].iterator();
                  while (i < stacks[j].height())
                  {
                     if (stackRow.hasNext())
                     {
                        //stackColor = (Integer) stacks[j].peek();
                        stackColor = (Integer) stackRow.next();
